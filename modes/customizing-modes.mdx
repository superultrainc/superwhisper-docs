...
## Overview
To customize a built-in mode, copy its AI instructions into a new [Custom Mode](custom) and adjust them there. Your customized version won't receive updates when the original mode improves, but you get a tailored mode without starting from scratch.
...
<Tip>For everything you can add to a Custom Mode, from context awareness to system audio capture, see [Custom Mode](custom).</Tip>