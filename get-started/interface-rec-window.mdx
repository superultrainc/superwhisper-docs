...
- Verify your selected audio input device in [Superwhisper's Sound tab](essential-settings#sound-and-microphone)
- Confirm microphone permissions in System Settings > Privacy & Security > Microphone

<Note>Exception: When using [system audio capture in a Custom mode](../modes/custom#audio-options), the waveform remains static. Your audio still records properly - you can verify this in the [History tab](history), which displays detailed information about each dictation session.</Note>
