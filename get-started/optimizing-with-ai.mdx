...
Your history keeps growing, so the optimization pays off on a schedule: new recognition mistakes become replacements, and your modes stay tuned to how you actually dictate. This launchd agent runs the skill through Claude Code every Monday at 9:00:

```bash
cat > ~/Library/LaunchAgents/com.superwhisper.optimize-weekly.plist <<'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key><string>com.superwhisper.optimize-weekly</string>
  <key>ProgramArguments</key>
  <array>
    <string>/bin/zsh</string>
    <string>-lc</string>
    <string>claude -p "Run the superwhisper-optimize skill non-interactively: fresh pass over my latest history, sensible defaults, additive changes only." --allowedTools "Skill,Bash,Read,Write,Edit" >> ~/Library/Logs/superwhisper-optimize.log 2>&amp;1</string>
  </array>
...
EOF
launchctl load ~/Library/LaunchAgents/com.superwhisper.optimize-weekly.plist
```

Each run appends its summary to `~/Library/Logs/superwhisper-optimize.log`, so you can review what changed. The skill only makes additive changes, and every run backs up `settings.json` first.

To stop the schedule:

```bash
launchctl unload ~/Library/LaunchAgents/com.superwhisper.optimize-weekly.plist
```

<Note>
The headless run pre-approves the listed tools for Claude Code. Skip the schedule if you'd rather review every change: running `/superwhisper-optimize` manually gives you the interactive questions and a summary each time.
</Note>
