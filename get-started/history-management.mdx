...
- Installs from before the default changed may still use `~/Documents/superwhisper/recordings`.
...
<Steps>
  ...
  <Step title="Add a Cleanup Rule">
    ...
    ```
    <Info>
    - If your recordings folder is in a different location (e.g. `$HOME/Documents/superwhisper/recordings` on older installs), replace `$HOME/superwhisper/recordings` with the correct path.
...
```
/usr/bin/find "$HOME/superwhisper/recordings" -mindepth 1 -maxdepth 1 -type d -name "[0-9]*" -mmin +1440 -print
```
