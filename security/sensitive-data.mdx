...
Superwhisper can run with no audio or text leaving your device. Pair a local voice model with a local language model, and every stage of a dictation stays on your machine. This page shows where your data goes in each configuration, and how to lock it down for regulated work.
...
- [Recommended configurations for healthcare and sensitive data](#recommended-configurations-for-healthcare-and-sensitive-data)
...
| --- | --- | --- | --- |
| Cohere Transcribe (Local) | Fast | Highest | The most accurate local model; first pick for sensitive work |
| Parakeet Multilanguage (Local) | Fastest | High | All-around workflows and dictation; the only local option on Windows ARM |
| Whisper Large v3 Turbo (Local) | Fast | High | A balance of speed and quality |

To change your voice model, open your mode settings and select from the local models listed under **Voice Model**.

<Tip>
For sensitive environments, **Cohere Transcribe** is the recommended starting point. It's the most accurate local model and fast enough for clinical or professional workflows.
</Tip>

### Local language models

Superwhisper supports local language models on macOS and Windows, letting AI post-processing run entirely on your device. Local language model options are available directly in your mode's **Language Model** settings.

<Note>
On Windows, local language models run on your GPU through Vulkan. Windows ARM (Snapdragon) devices don't support Vulkan yet, so they're limited to cloud language models or modes without AI post-processing.
</Note>

### Transcription only (no AI post-processing)

If your workflow only requires accurate transcription, with no AI formatting or transformation, you can use the **Voice to Text** mode, which outputs raw transcribed text with no language model involved. This is the simplest fully-local configuration.

<CardGroup cols={2}>
<Card title="Voice Models" icon="microphone" href="/models/voice">
Browse all local and cloud voice model options
</Card>
<Card title="Voice to Text Mode" icon="waveform" href="/modes/built-in#voice-to-text">
...
To sign a DPA or BAA with Superwhisper:
...
<CardGroup cols={2}>
<Card title="Enterprise Model Management" icon="server" href="/enterprise/configuration">
...
From the **Configuration** tab in your enterprise dashboard, you can disable Superwhisper's hosted cloud models entirely. When disabled:
...
<CardGroup cols={2}>
<Card title="Model Management" icon="sliders" href="/enterprise/configuration">
Restrict cloud models and add approved custom models
</Card>
<Card title="SAML SSO" icon="lock" href="/enterprise/saml-sso">
Set up single sign-on for your organization
</Card>
<Card title="SCIM Provisioning" icon="users" href="/enterprise/scim">
Automate user provisioning and deprovisioning
</Card>
<Card title="Enterprise Getting Started" icon="building" href="/enterprise/getting-started">
...
## Recommended configurations for healthcare and sensitive data

| Use Case | Voice Model | Language Model | Notes |
| --- | --- | --- | --- |
| Device only (air-gapped) | Cohere Transcribe (Local) or Parakeet Multilanguage (Local) | S1-mini (Local) | No data leaves the device |
| Healthcare | Nova Medical (Cloud), sign Superwhisper [BAA](https://docuseal.com/d/ehxSh1xq5RTGHh) | Claude 4.5 Sonnet (Anthropic, Cloud), sign Superwhisper [BAA](https://trust.mycroft.io/superwhisper) or Bring Your Own Keys | Covered by Superwhisper's zero-data-retention API terms or external provider agreement |
...
For sensitive environments, review your retention practices and [delete recordings](/get-started/history#delete-recordings) you no longer need.

### Custom vocabulary

Text replacements are stored locally and applied on your device after transcription. Vocabulary words work differently: they're sent alongside your audio as recognition hints, so with a cloud voice model they reach that provider under the same zero-data-retention API terms. With a local voice model, vocabulary never leaves your device.
...
If your organization requires a signed DPA or BAA with Superwhisper, you can [sign them here](https://docuseal.com/d/ehxSh1xq5RTGHh). See [Compliance](/security/compliance) for the Trust Center and SOC 2 report.

---

## Support

To get help configuring Superwhisper for your compliance environment:
