...
<img
  ...
  alt="Eight datasets feed a suite of sixteen models to produce one blended score from word error rate, response time, and recall, weighted 3 to 1 toward accuracy: Superwhisper S1 Voice 84, ElevenLabs Scribe v2 81, OpenAI GPT Transcribe 80, Wispr Flow raw 75, Deepgram Nova 3 75, Gemini 3.5 Transcribe 69"
/>

Every speed and accuracy figure in these docs comes from one benchmark suite: 16 speech models over 8 datasets, our own models and the ones we compete with, on the same audio and the same machines. The full leaderboard, with a page per model, is at [superwhisper.com/benchmarks](https://superwhisper.com/benchmarks).

This page explains how those numbers are produced, so you can judge whether they answer your question.
...
The leaderboard's headline score puts accuracy and response time into one number out of 100, weighted three to one in favour of accuracy. A transcript you have to correct is not a fast transcript.
...
## What the docs tables show instead

The Speed and Accuracy columns on [Voice models](/models/voice) and [Language models](/models/language) are relative ratings from 1 to 10, not benchmark output. They exist to compare models against each other at a glance, including the many models that aren't in the benchmark suite.
