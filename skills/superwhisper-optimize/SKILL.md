...
- Never send dictation content anywhere yourself: no web service, search engine, or URL, not even a snippet to "check a spelling". The CLI and the files you touch are all local. Note that the history you read still reaches whatever model provider powers this agent, under that provider's own data policy; that's inherent to running the skill, so read only what the job needs. If history contains credentials, addresses, or other sensitive data, don't repeat it in your output and never store it in vocabulary, replacements, or mode prompts.
